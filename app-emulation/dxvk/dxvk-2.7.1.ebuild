...
BDEPEND="
	${PYTHON_DEPS}
	dev-util/glslang
	arm64? ( dev-util/llvm-mingw64[arm64ec-pe(-),i686-pe(-)] )
	!crossdev-mingw? ( || (
...
src_configure() {
	if use arm64; then
		use crossdev-mingw || PATH=${BROOT}/usr/lib/llvm-mingw64/bin:${PATH}
	else
	...
	use arm64 || multilib-minimal_src_configure
}
...
src_compile() {
	if ! use arm64; then
		...
		for HOST in ${A64E_HOSTS[@]}; do
			( a64e_host_compile ${HOST} )
		done
	fi
}
