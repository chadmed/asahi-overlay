...
src_configure() {
	for CHOST in ${HOSTS[@]}; do
		(
			use unixlib && per_host_src_configure_unixlib
			setup_env
...
per_host_src_configure_unixlib() {
	mkdir "${WORKDIR}/${CHOST}-build-unixlib" || die
	pushd "${WORKDIR}/${CHOST}-build-unixlib" >/dev/null || die
	cmake -GNinja \
		-DCMAKE_CXX_COMPILER_WORKS=1 \
		-DCMAKE_BUILD_TYPE=Release \
		-DCMAKE_INSTALL_LIBDIR=/usr/lib/fex-xtajit \
		"${S}/Source/Windows/UnixLib" || die
	popd >/dev/null || die
}
...
src_compile() {
	for CHOST in ${HOSTS[@]}; do
		(
			use unixlib && per_host_src_compile_unixlib
			setup_env
...
src_install() {
	for CHOST in ${HOSTS[@]}; do
		(
			use unixlib && per_host_src_install_unixlib
			setup_env
