...
src_compile() {
	unset ARCH
	emake STRIP=: PREFIX=/usr KERNEL_TARBALL="${DISTDIR}/${KERNEL_VERSION}.tar.xz" $(tc-is-clang && echo LLVM=1)
}

src_install() {
	emake DESTDIR="${D}" PREFIX=/usr install
	# libkrun still expects libkrunfw.so.4 for some reason, but there
	# is no API or ABI here to worry about
	dosym -r /usr/lib64/libkrunfw.so.5 /usr/lib64/libkrunfw.so.4
}
